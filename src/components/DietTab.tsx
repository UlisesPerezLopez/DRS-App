  ...
  const getItemDisplayName = useCallback(
    (itm: SuggestedItem): string => {
      ...
      if (key.startsWith("recipes.")) {
        ...
        if (altResolved && altResolved !== `recipeDb.${key}.name`)
          return altResolved;
    ...
    <div className="px-4 pt-4 pb-28 space-y-4">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-4 shadow-sm">
        ...
        <select
          ...
          onChange={(e) =>
            setProfile({
              dietPreference: e.target.value as
                | "mediterranea"
                | "low-carb"
                | "vegetariana",
            })
          ...
          >
            <div className="flex justify-between items-center mb-3">
              <h3 className="font-bold text-lg">
                {t(
                  `meals.${m.meal === "Media Mañana" ? "mediaManana" : m.meal.toLowerCase()}`,
                  { defaultValue: m.meal },
                )}
              </h3>
                ...
                >
                  <div className="flex-1 pr-2">
                    ...
                    <div className="flex gap-2 text-[10px] text-slate-400 mt-0.5">
                      <span>{t("common.protein_abbr")}:{itm.protein}g</span>
                      <span>{t("common.carbs_abbr")}:{itm.carbs}g</span>
                      <span>{t("common.fat_abbr")}:{itm.fat}g</span>
                    </div>
                  </div>
                  <div className="flex items-center">
                    <span className="font-mono text-slate-500 whitespace-nowrap">
                      {itm.calories} <span className="text-[10px]">kcal</span>
                    </span>
                  </div>
                </li>
              ))}
