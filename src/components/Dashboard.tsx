...
export function Dashboard() {
  ...
  const workoutMin = Math.round(
    todayWorkouts.reduce((s, w) => s + w.durationSec, 0) / 60,
  );
  ...
  // Nutrition Logic
  const dailyMenu = generateDailyMenu(
    target,
    profile.dietPreference || "mediterranea",
    profile.goal,
  );
    ...
    <div className="px-4 pt-4 pb-4 space-y-4">
        ...
        <div className="group relative overflow-hidden bg-white dark:bg-slate-900 rounded-[2rem] p-4 border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none hover:shadow-2xl transition-all duration-300">
          ...
          <div className="relative z-10 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-bold text-slate-800 dark:text-white uppercase tracking-tight text-xs">
                {t("dashboard.nutrition_today")}
              </h2>
            </div>
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                {profile.dietPreference || "Mediterránea"}
              </p>
              <h3 className="text-base font-black text-slate-900 dark:text-white truncate">
                {t("dashboard.suggestion_label")}:{" "}
                {suggestion
                  ? suggestion.translationKey
                    ? t(`foodDb.${suggestion.translationKey}`)
                    : suggestion.name
                  : "..."}
              </h3>
              <p className="text-xs text-slate-500 mt-1">
                {suggestion
                  ? t("dashboard.approx_kcal", { amount: suggestion.calories })
                  : ""}
              </p>
            </div>
            <button
              onClick={() => navigate("/diet")}
              className="flex items-center gap-1 text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:gap-2 transition-all"
            >
        ...
        <div className="group relative overflow-hidden bg-white dark:bg-slate-900 rounded-[2rem] p-4 border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none hover:shadow-2xl transition-all duration-300">
          ...
          <div className="relative z-10 space-y-4">
            ...
            <div>
              ...
              <h3 className="text-base font-black text-slate-900 dark:text-white truncate">
                {mainRoutine
                  ? t(mainRoutine.translationKey)
                  : t("dashboard.rest_day")}
              </h3>
              <p className="text-xs text-slate-500 mt-1">
                {mainRoutine
                  ? t("dashboard.exercises_count", {
                      count: mainRoutine.exercises.length,
                    }) +
                    " · " +
                    userGoal
                  : t("dashboard.active_recovery")}
              </p>
            </div>
            <button
      ...
      <section className="rounded-3xl bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800 p-5 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-slate-800 dark:text-white tracking-tight">
              {t("dashboard.calorie_budget")}
            </h2>
          </div>
          <span className="text-[10px] font-black bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-lg text-slate-500 uppercase tracking-widest">
            {today}
          </span>
        </div>
        ...
        <div className="flex items-end justify-between">
          <div>
            ...
            <p
              className={`text-xs font-bold mt-2 uppercase tracking-wide ${over ? "text-rose-500" : "text-emerald-500"}`}
            >
              {over
                ? t("dashboard.kcalExceed", { amount: consumed - target })
                : t("dashboard.kcalRemain", { amount: remaining })}
            </p>
          </div>
        </div>
      ...
      <section className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-[10px] text-emerald-500 font-bold mt-1">
            {t("dashboard.target_label", { weight: profile.targetWeightKg })}
          </p>
        </div>
      </section>
      ...
      <div className="group rounded-[2rem] bg-white dark:bg-slate-900 p-6 border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none space-y-3 relative overflow-hidden text-left">
        ...
        <div className="flex items-center gap-2 relative z-10">
          ...
          <h3 className="font-black text-xs uppercase tracking-[0.2em] text-slate-800 dark:text-white">
            Análisis Diario
          </h3>
        </div>
