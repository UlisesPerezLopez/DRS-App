...
export function Recipes() {
  ...
  const fullMenu = useMemo(
    () => RECIPES.filter((r) => !isLowCostRecipe(r)),
    [],
  );
  ...
  function renderRecipeCard(r: (typeof RECIPES)[number]) {
    ...
    const ingredients = t(`${r.translationKey}.ingredients`, {
      returnObjects: true,
    }) as string[];
    const steps = t(`${r.translationKey}.steps`, {
      returnObjects: true,
    }) as string[];
            ...
            <div className="flex flex-wrap gap-x-3 gap-y-1 mt-1.5 text-xs text-slate-500">
              {r.cost && (
                <span className="flex items-center gap-1">
                  <Wallet size={12} /> {r.cost}
                </span>
              )}
              <span className="flex items-center gap-1">
                <Clock size={12} /> {r.time || `${r.prepTime} min`}
              </span>
              <span className="flex items-center gap-1">
                <Flame size={12} /> {r.kcal || r.calories} kcal
              </span>
            </div>
          </div>
          <ChevronDown
        ...
        >
          <div className="px-4 pb-4 pt-4 space-y-4">
            <div className="grid grid-cols-3 gap-2 text-center">
              ...
              />
              <Macro
                label={t("recipes.carbsLabel")}
                value={`${r.carbs} g`}
                color="amber"
              />
              <Macro
                label={t("recipes.fatLabel")}
                value={`${r.fat} g`}
                color="rose"
              />
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">
                {t("recipes.lowCostTitle")}
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {t("recipes.lowCostDesc", { count: lowCost.length })}
              </p>
            </div>
          </div>
          <ChevronDown
      ...
      <section className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 overflow-hidden shadow-sm">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">
                {t("recipes.fullMenuTitle")}
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {t("recipes.fullMenuDesc", { count: fullMenu.length })}
              </p>
            </div>
          </div>
          <ChevronDown
